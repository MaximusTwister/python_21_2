...
from utils import query_flights, encode_jwt, decode_jwt, get_salt
...
def signup():
    ...
    if form.validate_on_submit():
        if users_coll.find_one({'email': form.email.data}):     # проверка есть ли пользователь в базе
            ...
            # New
            salt = get_salt()
            hashed_password = sha256(form.password.data.encode() + salt.encode()).hexdigest()
            environ.setdefault(form.email.data, salt)
